...
import { getCategoryListData, deleteCategory, updateCategory, addCategory } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    show (index) {
      ...
      this.$Modal.info({
        title: '详情',
        content: `分类名称：${this.listData[index].name}`
      })
    },
    ...
    deleteBatch () {
      ...
      if (this.selection && this.selection.length > 0) {
        for (var index = 0; index < this.selection.length; index++) {
          let categoryObj = this.selection[index]
          if (categoryObj) {
