      ...
      <div style='text-align:center'>
        <Form :model='formItem' :label-width='80'>
          ...
          <Form-item label='是否提交过DAPP*'>
            <RadioGroup v-model="currDate.hasSubmittedDapp">
...
import { getDappPromotedApplicationListData, deleteDappPromotedApplication, updateDappPromotedApplication, addDappPromotedApplication } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      getDappPromotedApplicationListData(params).then(res => {
    ...
    remove (tagId, index) {
      let params = {}
      params.ids = tagId
      deleteDappPromotedApplication(params).then(res => {
        let responseData = res.data
        if (!responseData) {
          return
        }
    ...
    saveBatch (dappPromotedApplication) {
      if (!dappPromotedApplication) {
        return
      }
      this.loading = true
      let params = {}
      // params.name = dappPromotedApplication.name
      // params = Object.assign({}, dappPromotedApplication)
      // this.transferParams(params) // For GET method
      if (this.currIndex !== -1) { // 编辑模式
        params.id = dappPromotedApplication.id
        updateDappPromotedApplication(dappPromotedApplication).then(res => {
          let responseData = res.data
          if (!responseData) {
            return
          }
        ...
        addDappPromotedApplication(dappPromotedApplication).then(res => {
          let responseData = res.data
          if (!responseData) {
            return
          }
